...
n5=length(bloc5[,1])
n=n1+n2+n3+n4+n5
colnames(bloc1)[-c(30,31,32,33,34)]=paste0("x", 1:29)
colnames(bloc2)[-c(30,31,32,33,34)]=paste0("x", 1:29)
colnames(bloc3)[-c(30,31,32,33,34)]=paste0("x", 1:29)
colnames(bloc4)[-c(30,31,32,33,34)]=paste0("x", 1:29)
colnames(bloc5)[-c(30,31,32,33,34)]=paste0("x", 1:29)
...
for(i in 0:nombre_de_lambdas){
  model=glmmLasso(PMA ~MeanRR+sdRR+Rmssd+maxRR+minRR+Skewness+Kurtosis+
                    ...
                    r_HVG+nbHighqual, rnd = list(ID=~1),lambda = i,data = data)
...
coeffs=glmmLasso(PMA ~MeanRR+sdRR+Rmssd+maxRR+minRR+Skewness+Kurtosis+
                  AC+DC+LF+HF+LFnu+HFnu+LF_HF+SD1+SD2+SampEn+ApEn+Alpha1+
                  Alpha2+MD_VG+C_VG+Tr_VG+r_VG+MD_HVG+C_HVG+Tr_HVG+
                  r_HVG+nbHighqual, rnd = list(ID=~1),lambda = resultats$lambdas[1],data = data)$coefficients
print(names(coeffs)[which(abs(round(coeffs,chiffres_significatifs))>0)])
#lamda_opti = 804
#MSE = 4.480782
#effets fixes selectionnés
#[1] "(Intercept)" "sdRR"        "AC"          "DC"          "LF"          "SD2"        
#[7] "ApEn"        "Alpha1"      "C_VG"        "r_VG"        "MD_HVG"      "C_HVG"      
#[13] "Tr_HVG"      "r_HVG" 