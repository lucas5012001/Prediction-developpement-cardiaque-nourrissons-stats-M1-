...
features_i = which(colnames(data)%in%setdiff(colnames(data),c("PMA","Day","ID","GA","Term")))
PMA_i = which(colnames(data)=="PMA")
groups_i = which(colnames(data) %in% c("ID"))
#################################################
#grille de recherche des hyperparametres
hyper_grid <- expand.grid(
  num_iter = seq(180,210,2),
  min_data_in_leaf = seq(25,30,1),
  MSE1   = 0,
  MSE2   = 0,
  MSE3   = 0,
  MSE4   = 0,
