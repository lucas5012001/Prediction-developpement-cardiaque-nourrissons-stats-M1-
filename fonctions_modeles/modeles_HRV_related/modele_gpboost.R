...
modele_gpboost = function(train,test){
  features_i = which(colnames(train)%in%setdiff(colnames(train),c("PMA","Day","ID","Term","GA")))
  PMA_i = which(colnames(train)=="PMA")
  groups_i = which(colnames(train) %in% c("ID"))
  ...
  bst <- gpboost(data = X_train, label = Y_train, gp_model = gp_model,objective = "regression_l2",verbose = 0,params = list(num_iterations=204,min_data_in_leaf = 26,learning_rate = 0.05))
