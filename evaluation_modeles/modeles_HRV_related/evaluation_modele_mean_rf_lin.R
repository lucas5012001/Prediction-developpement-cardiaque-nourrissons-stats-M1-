...
#################################################
eval1=modele_mean_rf_lin(train=rbind(bloc5,bloc2,bloc3,bloc4),test=bloc1)
eval2=modele_mean_rf_lin(train=rbind(bloc1,bloc5,bloc3,bloc4),test=bloc2)
eval3=modele_mean_rf_lin(train=rbind(bloc1,bloc2,bloc5,bloc4),test=bloc3)
eval4=modele_mean_rf_lin(train=rbind(bloc1,bloc2,bloc3,bloc5),test=bloc4)
eval5=modele_mean_rf_lin(train=rbind(bloc1,bloc2,bloc3,bloc4),test=bloc5)
#################################################
n1=length(bloc1[,1])
n2=length(bloc2[,1])
n3=length(bloc3[,1])
...
ggplot(data, aes(x = PMA_estim, y = PMA, color = Term)) + 
  ...
  labs(color = "Term / Groupe", 
       linetype = "Droite", 
       title = "Relation entre PMA réel et PMA estimé mean rf lin") +
  scale_linetype_manual(values = c("solid")) +
  theme_classic()
#enregistrement
results[2:7] <- lapply(results[2:7], as.numeric)
results[2:7] = round(results[2:7],3)
htmlwidgets::saveWidget(datatable(results), "plots/eval_modele_mean_rf_lin.html")
