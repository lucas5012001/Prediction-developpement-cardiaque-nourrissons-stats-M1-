...
for(i in 1:length(loo_list_mediane)){
  ...
  predictions_mediane[loo_list_mediane[[i]]] = modele_combine(data[-loo_list_mediane[[i]],],data[loo_list_mediane[[i]],])[,2]
}
...
ggplot(data, aes(x = PMA_estim, y = PMA, color = Term)) + 
  ...
  labs(color = "Term / Groupe", 
       linetype = "Droite", 
       title = "Relation entre PMA réel et PMA estimé combine loo (data)") +
...
for(i in 1:length(loo_list_healthy)){
  ...
  predictions_healthy[loo_list_healthy[[i]]] = modele_combine(healthy[-loo_list_healthy[[i]],],healthy[loo_list_healthy[[i]],])[,2]
}
...
ggplot(healthy, aes(x = PMA_estim, y = PMA, color = Term)) + 
  ...
  labs(color = "Term / Groupe", 
       linetype = "Droite", 
       title = "Relation entre PMA réel et PMA estimé combine loo (healthy)") +
  scale_linetype_manual(values = c("solid")) +
  theme_classic()
##############################
MSE_mediane
